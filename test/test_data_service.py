...
async def test_enrich_dataset_with_dataset_entity(data_service, mock_opengin_service):
    ...
    with patch(
        ...
        await data_service.enrich_dataset(
            dataset_dictionary=dataset_dictionary, dataset=dataset
        )
...
async def test_enrich_dataset_with_dataset_relation(data_service, mock_opengin_service):
    ...
    with patch(
        ...
        await data_service.enrich_dataset(
            dataset_dictionary=dataset_dictionary, dataset_relation=dataset_relation
        )
...
async def test_enrich_category_with_category_entity(data_service):
    ...
    category = Entity(
        ...
        kind=Kind(
            major=KindMajorEnum.CATEGORY.value,
            minor=KindMinorEnum.PARENT_CATEGORY.value,
        ),
    )
    ...
    with patch(
        ...
        await data_service.enrich_category(
            categories_dictionary=categories_dictionary, category=category
        )
...
@pytest.mark.asyncio
async def test_enrich_category_with_category_relation(
    data_service, mock_opengin_service
):
    ...
    mock_entity = Entity(
        ...
        name="encoded_category_name",
        kind=Kind(
            major=KindMajorEnum.CATEGORY.value, minor=KindMinorEnum.CHILD_CATEGORY.value
        ),
    )
...
async def test_enrich_category_with_internal_error(data_service, mock_opengin_service):
    ...
    with pytest.raises(InternalServerError) as exc_info:
        await data_service.enrich_category(
            categories_dictionary={}, category_relation=category_relation
        )
...
@pytest.mark.asyncio
async def test_fetch_data_catalog_with_entity_id_and_relations(
    data_service, mock_opengin_service
):
    ...
    category_relations = [
        Relation(
            relatedEntityId="cat_1",
            name=RelationNameEnum.AS_CATEGORY.value,
            direction=RelationDirectionEnum.OUTGOING.value,
        ),
        Relation(
            relatedEntityId="cat_2",
            name=RelationNameEnum.AS_CATEGORY.value,
            direction=RelationDirectionEnum.OUTGOING.value,
        ),
    ]
    ...
    mock_opengin_service.get_entities.side_effect = [
        [
            Entity(
                ...
                kind=Kind(
                    major=KindMajorEnum.CATEGORY.value,
                    minor=KindMinorEnum.CHILD_CATEGORY.value,
                ),
            )
        ],
        [
...
@pytest.mark.asyncio
async def test_fetch_data_catalog_with_entity_id_no_relations(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_data_catalog_with_only_categories(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_data_catalog_with_only_datasets(
    data_service, mock_opengin_service
):
    ...
    mock_opengin_service.get_entities.side_effect = [
        [
            Entity(
                id="ds_1",
                name="encoded_ds_1",
                kind=Kind(major=KindMajorEnum.DATASET.value, minor="dataset"),
            )
        ],
        [
            Entity(
...
@pytest.mark.asyncio
async def test_fetch_data_catalog_with_internal_error(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_available_years_success(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_available_years_single_year(
    data_service, mock_opengin_service
):
    ...
    with patch(
        ...
        result = await data_service.fetch_dataset_available_years(
...
@pytest.mark.asyncio
async def test_fetch_dataset_available_years_with_missing_created_date(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_available_years_with_internal_error(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_available_years_multiple_years_sorted(
    data_service, mock_opengin_service
):
...
async def test_fetch_data_attributes_success(data_service, mock_opengin_service):
    ...
    with (
        patch(
            "src.services.data_service.Util.decode_protobuf_attribute_name",
            return_value="decoded_dataset_name",
        ),
        patch(
            "src.services.data_service.Util.transform_data_for_chart",
            return_value=mock_formatted_data,
        ),
    ):
    ...
    mock_opengin_service.get_attributes.assert_called_once_with(
        category_id="category_456", dataset_name="decoded_dataset_name"
    )
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_dataset_not_found(
    data_service, mock_opengin_service
):
    ...
    mock_opengin_service.fetch_relation.return_value = [
        Relation(
            relatedEntityId="category_123", name=RelationNameEnum.IS_ATTRIBUTE.value
        )
    ]
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_no_relations_found(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_with_empty_attributes(
    data_service, mock_opengin_service
):
    ...
    mock_relation = Relation(
        relatedEntityId="category_empty", name=RelationNameEnum.IS_ATTRIBUTE.value
    )
    ...
    with (
        ...
        patch(
            "src.services.data_service.Util.transform_data_for_chart",
            return_value=mock_formatted_empty,
        ),
    ):
        result = await data_service.fetch_data_attributes(dataset_id=dataset_id)
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_with_get_entities_error(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_with_fetch_relation_error(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_with_get_attributes_error(
    data_service, mock_opengin_service
):
    ...
    mock_opengin_service.get_entities.return_value = [mock_entity]
    mock_opengin_service.fetch_relation.return_value = [mock_relation]
    mock_opengin_service.get_attributes.side_effect = Exception(
        "Attributes fetch failed"
    )
...
@pytest.mark.asyncio
async def test_fetch_data_attributes_with_multiple_relations(
    data_service, mock_opengin_service
):
    ...
    mock_relations = [
        Relation(
            relatedEntityId="category_first", name=RelationNameEnum.IS_ATTRIBUTE.value
        ),
        Relation(
            relatedEntityId="category_second", name=RelationNameEnum.IS_ATTRIBUTE.value
        ),
    ]
    ...
    mock_opengin_service.get_attributes.assert_called_once_with(
        category_id="category_first", dataset_name="multi_dataset"
    )
...
async def test_enrich_category_with_lock_prevents_race_condition(data_service):
    ...
    categories = [
        Entity(
            id=f"cat_{i}",
            name=f"encoded_category_name_{i % 3}",
            kind=Kind(
                major=KindMajorEnum.CATEGORY.value,
                minor=KindMinorEnum.PARENT_CATEGORY.value,
            ),
    ...
    assert (
        len(categories_dictionary) == 3
    )  # Should have 3 unique category names (0, 1, 2)
...
@pytest.mark.asyncio
async def test_enrich_dataset_with_lock_prevents_race_condition(
    data_service, mock_opengin_service
):
    ...
    datasets = [
        Entity(
            id=f"ds_{i}",
            name=f"dataset_name_{i % 2}-202{i % 2}",
            kind=Kind(
                major=KindMajorEnum.DATASET.value, minor=KindMinorEnum.TABULAR.value
            ),
        )
    ...
    with patch(
        "src.services.data_service.Util.decode_protobuf_attribute_name",
        side_effect=[f"Employment-202{'0' if i % 2 == 0 else '1'}" for i in range(8)],
    ):
        tasks = [
            data_service.enrich_dataset(
                dataset_dictionary=dataset_dictionary, dataset=ds
            )
            for ds in datasets
        ]
        await asyncio.gather(*tasks)
...
@pytest.mark.asyncio
async def test_find_root_department_or_minister_found_department(
    data_service, mock_opengin_service
):
    ...
    mock_department = Entity(
        ...
        kind=Kind(
            major=KindMajorEnum.CATEGORY.value, minor=KindMinorEnum.DEPARTMENT.value
        ),
    )
    ...
    mock_opengin_service.get_entities.assert_called_once_with(
        entity=Entity(id=category_id)
    )


@pytest.mark.asyncio
async def test_find_root_department_or_minister_found_minister(
    data_service, mock_opengin_service
):
    ...
    mock_minister = Entity(
        ...
        kind=Kind(
            major=KindMajorEnum.CATEGORY.value, minor=KindMinorEnum.STATE_MINISTER.value
        ),
    )
...
@pytest.mark.asyncio
async def test_find_root_department_or_minister_recursive_traversal(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_find_root_department_or_minister_no_parent_found(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_find_root_department_or_minister_with_internal_error(
    data_service, mock_opengin_service
):
    """Test find_root_department_or_minister handles internal errors"""
    category_id = "error_category_222"

    mock_opengin_service.get_entities.side_effect = Exception(
...
@pytest.mark.asyncio
async def test_fetch_dataset_root_success_with_department(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_root_success_with_minister(
    data_service, mock_opengin_service
):
    ...
    mock_opengin_service.get_entities.side_effect = [[mock_category], [mock_minister]]

    with patch(
        "src.services.data_service.Util.decode_protobuf_attribute_name",
        return_value="Minister of Health",
    ):
        result = await data_service.fetch_dataset_root(dataset_id)
...
@pytest.mark.asyncio
async def test_fetch_dataset_root_no_root_entity_found(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_root_with_fetch_relation_error(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_root_with_find_root_error(
    data_service, mock_opengin_service
):
...
@pytest.mark.asyncio
async def test_fetch_dataset_root_multiple_relations_uses_first(
    data_service, mock_opengin_service
):
    ...
    mock_relations = [
        Relation(
            relatedEntityId=category_id_1,
            name=RelationNameEnum.IS_ATTRIBUTE.value,
            direction=RelationDirectionEnum.INCOMING.value,
        ),
        Relation(
            relatedEntityId=category_id_2,
            name=RelationNameEnum.IS_ATTRIBUTE.value,
            direction=RelationDirectionEnum.INCOMING.value,
        ),
    ]
