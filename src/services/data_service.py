...
class DataService:
    ...
    async def enrich_dataset(
        ...
        dataset: Entity = None,
        dataset_relation: Relation = None,
    ):
        """
        ...
        try:
            ...
            if dataset_relation:
                ...
                datasets = await self.opengin_service.get_entities(
            ...
            async with self.lock:
                dataset_dictionary.setdefault(actual_name_title_case, set()).add(
                    dataset.id
                )
    ...
    async def enrich_category(
        ...
        category: Entity = None,
        category_relation: Relation = None,
    ):
        ...
        try:
            ...
            if category_relation:
                ...
                categories = await self.opengin_service.get_entities(
            ...
            async with self.lock:
                categories_dictionary.setdefault(actual_name_title_case, set()).add(
                    category.id
                )
    ...
    def convert_dict_to_list(dictionary: dict, key_name: str, value_name: str) -> list:
        ...
        """
        list_of_dicts = [
            {key_name: category, value_name: ids}
    ...
    async def fetch_data_catalog(self, category_ids: list[str] = None):
        ...
        try:
            if not category_ids:
                ...
                parentCategories = await self.opengin_service.get_entities(
                    entity=entity
                )

                enrich_category_task = [
                    self.enrich_category(
                ...
                categories = self.convert_dict_to_list(
                    categories_dictionary, "name", "categoryIds"
                )

                return DataCatalogResponse(categories=categories, datasets=[]).model_dump()
                ...
                dataset_relation_instance = Relation(
                    name=RelationNameEnum.IS_ATTRIBUTE.value,
                    direction=RelationDirectionEnum.OUTGOING.value,
                )

                fetch_category_relation_tasks = [
                    self.opengin_service.fetch_relation(
                        entityId=category_id, relation=category_relation_instance
                    )
                    for category_id in category_ids
                ]
                fetch_dataset_relation_tasks = [
                    self.opengin_service.fetch_relation(
                        entityId=category_id, relation=dataset_relation_instance
                    )
                    for category_id in category_ids
                ]
                ...
                dataset_enrich_tasks = [
                    self.enrich_dataset(
                        dataset_relation=relation, dataset_dictionary=dataset_dictionary
                    )
                ...
                datasets = self.convert_dict_to_list(
                    dataset_dictionary, "name", "datasetIds"
                )

                return DataCatalogResponse(categories=categories, datasets=datasets).model_dump()
    ...
    async def fetch_dataset_available_years(self, dataset_ids: list[str]):
        ...
        try:
            ...
            # parallel execution of tasks
            dataset_entity_tasks = [
                self.opengin_service.get_entities(entity=entity)
    ...
    async def fetch_data_attributes(self, dataset_id: str):
        ...
        try:
            ...
            dataset_entity_result, dataset_relations_result = await asyncio.gather(
                ...
                self.opengin_service.fetch_relation(
                    entityId=dataset_id, relation=dataset_relation
                ),
            )
            ...
            attributes = await self.opengin_service.get_attributes(
                category_id=category_id, dataset_name=dataset_name
            )
    ...
    async def fetch_dataset_root(self, dataset_id: str):
        ...
        try:
            ...
            relations = await self.opengin_service.fetch_relation(
                entityId=dataset_id, relation=relation_instance
            )
    ...
    async def fetch_dataset_categories(self, dataset_id: str):
        ...
        try:
            ...
            dataset_entity = Entity(id=dataset_id)
            dataset_results = await self.opengin_service.get_entities(
    ...
    async def _collect_category_hierarchy(self, category_id: str) -> list:
        ...
        while current_id:
            ...
            category_results = await self.opengin_service.get_entities(
            ...
            if current_category.kind and current_category.kind.minor in [
            ...
            parent_relations = await self.opengin_service.fetch_relation(
                entityId=current_id, relation=relation_instance
            )
    ...
    async def find_root_department_or_minister(self, category_id: str):
        ...
        try:
            ...
            parent_relations = await self.opengin_service.fetch_relation(
                entityId=category_id, relation=relation_instance
            )
            ...
            logger.error(
                f"Failed to find root department, state minister or cabinet minister for category {category_id}: {e}"
            )
            raise InternalServerError("An unexpected error occurred") from e
